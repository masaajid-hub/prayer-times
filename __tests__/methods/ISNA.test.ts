/**
 * ISNA method accuracy tests
 * Validates against official ISNA calculation authority data
 */

import { test, expect } from "bun:test";
import { PrayerTimeCalculator } from "../../src/api/prayer-times";
import isnaTestData from "../../test-data/ISNA.json";
...
test("ISNA method - metadata validation", () => {
  expect(isnaTestData.metadata.authority).toBe(
    "Islamic Society of North America (ISNA)"
  );
  expect(isnaTestData.metadata.method).toBe("ISNA");
  expect(isnaTestData.metadata.location).toBe("Mississauga, Canada");
  expect(isnaTestData.testCases.length).toBeGreaterThan(0);

  console.log(
    `ISNA test data: ${isnaTestData.testCases.length} test cases (Official Authority Data - High Quality (±5min max))`
  );
});

test("ISNA method - sample accuracy validation", () => {
  const testCases = isnaTestData.testCases.slice(0, 10) as TestCase[];
  const results: {
    date: string;
    prayer: string;
    status: string;
  ...
  for (const testCase of testCases) {
    ...
    const config = {
      method: testCase.input.method as "ISNA",
  ...
  console.log(`ISNA Method Accuracy Analysis (${totalTests} tests):`);
  console.log(
...
test("ISNA method - full dataset statistical analysis", () => {
  const testCases = isnaTestData.testCases as TestCase[];
  let totalDifference = 0;
  let maxDifference = 0;
  let testCount = 0;

  const coords = isnaTestData.metadata.coordinates;
  ...
  const config = {
    ...
    location,
    timezone: isnaTestData.metadata.timezone,
  };
  ...
  console.log(`ISNA Statistical Analysis (${testCount} prayer times):`);
...
test("ISNA method - edge cases and seasonal variation", () => {
  const coords = isnaTestData.metadata.coordinates;
  const location: [number, number] = Array.isArray(coords)
    ? [coords[0], coords[1]]
    : [coords.latitude, coords.longitude];
  ...
  console.log(
    `✅ ISNA method passed edge case validation for ${keyDates.length} key dates`
  );
});
