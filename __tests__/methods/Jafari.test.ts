...
import jafariTestData from "../../test-data/Jafari.json";
...
test("Jafari method - metadata validation", () => {
  expect(jafariTestData.metadata.method).toBe("Jafari");
  expect(jafariTestData.testCases.length).toBeGreaterThan(0);

  console.log(
    `Jafari test data: ${jafariTestData.testCases.length} test cases (Reference Implementation - Exact Match Expected (±1min max))`
  );
});

test("Jafari method - sample accuracy validation", () => {
  const testCases = jafariTestData.testCases.slice(0, 10) as TestCase[];
  const results: {
    date: string;
    prayer: string;
    status: string;
  ...
  for (const testCase of testCases) {
    ...
    const config = {
      method: testCase.input.method as "Jafari",
  ...
  console.log(`Jafari Method Accuracy Analysis (${totalTests} tests):`);
  console.log(
...
test("Jafari method - edge cases and seasonal variation", () => {
  const coords = jafariTestData.metadata.coordinates;
  const location: [number, number] = Array.isArray(coords)
    ? [coords[0], coords[1]]
    : [coords.latitude, coords.longitude];

  const config = {
    method: "Jafari" as const,
    location,
    timezone: jafariTestData.metadata.timezone,
  };
  ...
  console.log(
    `✅ Jafari method passed edge case validation for ${keyDates.length} key dates`
  );
});
