/**
 * Moonsighting method accuracy tests
...
import moonsightingTestData from "../../test-data/Moonsighting.json";
...
// Special Official Data - Moonsighting (±5min max) configuration
const TEST_CONFIG = {
  MAX_TOLERANCE: 5,
  TIER1_ACCURACY_TARGET: 0.2,
  TIER2_ACCURACY_TARGET: 0.4,
  TIER3_ACCURACY_TARGET: 0.7,
...
test("Moonsighting method - metadata validation", () => {
  expect(moonsightingTestData.metadata.authority).toBe(
    "Moonsighting.com (UK Islamic Moonsighting Committee)"
  );
  expect(moonsightingTestData.metadata.method).toBe("Moonsighting");
  expect(moonsightingTestData.metadata.location).toBe("London, United Kingdom");
  expect(moonsightingTestData.testCases.length).toBeGreaterThan(0);

  console.log(
    `Moonsighting test data: ${moonsightingTestData.testCases.length} test cases (Special Official Data - Moonsighting (±5min max))`
  );
});

test("Moonsighting method - sample accuracy validation", () => {
  const testCases = moonsightingTestData.testCases.slice(0, 10) as TestCase[];
  const results: {
    date: string;
    prayer: string;
    status: string;
  ...
  for (const testCase of testCases) {
    ...
    const config = {
      method: testCase.input.method as "Moonsighting",
  ...
  console.log(`Moonsighting Method Accuracy Analysis (${totalTests} tests):`);
  console.log(
...
test("Moonsighting method - edge cases and seasonal variation", () => {
  const coords = moonsightingTestData.metadata.coordinates;
  const location: [number, number] = Array.isArray(coords)
    ? [coords[0], coords[1]]
    : [coords.latitude, coords.longitude];

  const config = {
    method: "Moonsighting" as const,
    location,
    timezone: moonsightingTestData.metadata.timezone,
  ...
  console.log(
    `✅ Moonsighting method passed edge case validation for ${keyDates.length} key dates`
  );
});
