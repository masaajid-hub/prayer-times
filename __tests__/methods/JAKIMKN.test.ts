...
import jakimknTestData from "../../test-data/JAKIMKN.json";
...
test("JAKIMKN method - metadata validation", () => {
  expect(jakimknTestData.metadata.authority).toBe("Jabatan Kemajuan Islam Malaysia (JAKIM) - Kelantan");
  expect(jakimknTestData.metadata.method).toBe("JAKIMKN");
  expect(jakimknTestData.metadata.location).toBe("Rantau Panjang, Kelantan, Malaysia");
  expect(jakimknTestData.testCases.length).toBeGreaterThan(0);

  console.log(`JAKIMKN test data: ${jakimknTestData.testCases.length} test cases (Official Authority Data - High Quality (±5min max))`);
});

test("JAKIMKN method - sample accuracy validation", () => {
  const testCases = jakimknTestData.testCases.slice(0, 10) as TestCase[];
  ...
  for (const testCase of testCases) {
    ...
    const config = {
      method: testCase.input.method as "JAKIMKN",
  ...
  console.log(`JAKIMKN Method Accuracy Analysis (${totalTests} tests):`);
...
test("JAKIMKN method - full dataset statistical analysis", () => {
  const testCases = jakimknTestData.testCases as TestCase[];
  let totalDifference = 0;
  let maxDifference = 0;
  let testCount = 0;

  const coords = jakimknTestData.metadata.coordinates;
  const location: [number, number] = Array.isArray(coords)
    ? [coords[0], coords[1]]
    : [coords.latitude, coords.longitude];

  const config = {
    method: "JAKIMKN" as const,
    location,
    timezone: jakimknTestData.metadata.timezone
  };
  ...
  console.log(`JAKIMKN Statistical Analysis (${testCount} prayer times):`);
...
test("JAKIMKN method - edge cases and seasonal variation", () => {
  const coords = jakimknTestData.metadata.coordinates;
  const location: [number, number] = Array.isArray(coords)
    ? [coords[0], coords[1]]
    : [coords.latitude, coords.longitude];
  ...
  console.log(`✅ JAKIMKN method passed edge case validation for ${keyDates.length} key dates`);
});