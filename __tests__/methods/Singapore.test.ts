/**
 * Singapore method accuracy tests
 * Validates against MUIS Singapore official calculation data
 */

import { test, expect } from "bun:test";
import { PrayerTimeCalculator } from "../../src/api/prayer-times";
import singaporeTestData from "../../test-data/Singapore.json";
...
// Official Authority Data - Standard Quality (±5min max) configuration
const TEST_CONFIG = {
  MAX_TOLERANCE: 5,
  TIER1_ACCURACY_TARGET: 0.4,
  TIER2_ACCURACY_TARGET: 0.7,
  TIER3_ACCURACY_TARGET: 0.9,
...
test("Singapore method - metadata validation", () => {
  expect(singaporeTestData.metadata.authority).toBe(
    "Majlis Ugama Islam Singapura (MUIS)"
  );
  expect(singaporeTestData.metadata.method).toBe("Singapore");
  expect(singaporeTestData.metadata.location).toBe("Singapore");
  expect(singaporeTestData.testCases.length).toBeGreaterThan(0);

  console.log(
    `Singapore test data: ${singaporeTestData.testCases.length} test cases (Official Authority Data - Standard Quality (±5min max))`
  );
});

test("Singapore method - sample accuracy validation", () => {
  const testCases = singaporeTestData.testCases.slice(0, 10) as TestCase[];
  const results: {
    date: string;
    prayer: string;
    status: string;
  ...
  for (const testCase of testCases) {
    ...
    const config = {
      method: testCase.input.method as "Singapore",
  ...
  console.log(`Singapore Method Accuracy Analysis (${totalTests} tests):`);
  console.log(
...
test("Singapore method - full dataset statistical analysis", () => {
  const testCases = singaporeTestData.testCases as TestCase[];
  let totalDifference = 0;
  let maxDifference = 0;
  let testCount = 0;

  const coords = singaporeTestData.metadata.coordinates;
  const location: [number, number] = Array.isArray(coords)
    ? [coords[0], coords[1]]
    : [coords.latitude, coords.longitude];

  const config = {
    method: "Singapore" as const,
    location,
    timezone: singaporeTestData.metadata.timezone,
  };
  ...
  console.log(`Singapore Statistical Analysis (${testCount} prayer times):`);
...
test("Singapore method - edge cases and seasonal variation", () => {
  const coords = singaporeTestData.metadata.coordinates;
  const location: [number, number] = Array.isArray(coords)
    ? [coords[0], coords[1]]
    : [coords.latitude, coords.longitude];
  ...
  console.log(
    `✅ Singapore method passed edge case validation for ${keyDates.length} key dates`
  );
});
