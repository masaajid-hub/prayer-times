/**
 * Turkey method accuracy tests
 * Validates against Turkey Diyanet official calculation data
 */

import { test, expect } from "bun:test";
import { PrayerTimeCalculator } from "../../src/api/prayer-times";
import turkeyTestData from "../../test-data/Turkey.json";
...
test("Turkey method - metadata validation", () => {
  expect(turkeyTestData.metadata.authority).toBe(
    "Diyanet İşleri Başkanlığı (Turkish Religious Affairs)"
  );
  expect(turkeyTestData.metadata.method).toBe("Turkey");
  expect(turkeyTestData.metadata.location).toBe("Ankara, Turkey");
  expect(turkeyTestData.testCases.length).toBeGreaterThan(0);

  console.log(
    `Turkey test data: ${turkeyTestData.testCases.length} test cases (Official Authority Data - Standard Quality (±5min max))`
  );
});

test("Turkey method - sample accuracy validation", () => {
  const testCases = turkeyTestData.testCases.slice(0, 10) as TestCase[];
  const results: {
    date: string;
    prayer: string;
    status: string;
  ...
  for (const testCase of testCases) {
    ...
    const config = {
      method: testCase.input.method as "Turkey",
  ...
  console.log(`Turkey Method Accuracy Analysis (${totalTests} tests):`);
  console.log(
...
test("Turkey method - full dataset statistical analysis", () => {
  const testCases = turkeyTestData.testCases as TestCase[];
  let totalDifference = 0;
  let maxDifference = 0;
  let testCount = 0;

  const coords = turkeyTestData.metadata.coordinates;
  const location: [number, number] = Array.isArray(coords)
    ? [coords[0], coords[1]]
    : [coords.latitude, coords.longitude];

  const config = {
    method: "Turkey" as const,
    location,
    timezone: turkeyTestData.metadata.timezone,
  };
  ...
  console.log(`Turkey Statistical Analysis (${testCount} prayer times):`);
...
test("Turkey method - edge cases and seasonal variation", () => {
  const coords = turkeyTestData.metadata.coordinates;
  const location: [number, number] = Array.isArray(coords)
    ? [coords[0], coords[1]]
    : [coords.latitude, coords.longitude];
  ...
  console.log(
    `✅ Turkey method passed edge case validation for ${keyDates.length} key dates`
  );
});
