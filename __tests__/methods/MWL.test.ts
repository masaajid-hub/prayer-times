...
import mwlTestData from "../../test-data/MWL.json";
...
test("MWL method - metadata validation", () => {
  expect(mwlTestData.metadata.method).toBe("MWL");
  expect(mwlTestData.testCases.length).toBeGreaterThan(0);

  console.log(
    `MWL test data: ${mwlTestData.testCases.length} test cases (Reference Implementation - Exact Match Expected (±1min max))`
  );
});

test("MWL method - sample accuracy validation", () => {
  const testCases = mwlTestData.testCases.slice(0, 10) as TestCase[];
  const results: {
    date: string;
    prayer: string;
    status: string;
  ...
  for (const testCase of testCases) {
    ...
    const config = {
      method: testCase.input.method as "MWL",
  ...
  console.log(`MWL Method Accuracy Analysis (${totalTests} tests):`);
  console.log(
...
test("MWL method - edge cases and seasonal variation", () => {
  const coords = mwlTestData.metadata.coordinates;
  ...
  const config = {
    method: "MWL" as const,
    location,
    timezone: mwlTestData.metadata.timezone,
  };
  ...
  console.log(
    `✅ MWL method passed edge case validation for ${keyDates.length} key dates`
  );
});
