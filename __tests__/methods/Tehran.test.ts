...
import tehranTestData from "../../test-data/Tehran.json";
...
test("Tehran method - metadata validation", () => {
  expect(tehranTestData.metadata.method).toBe("Tehran");
  expect(tehranTestData.testCases.length).toBeGreaterThan(0);

  console.log(
    `Tehran test data: ${tehranTestData.testCases.length} test cases (Reference Implementation - Exact Match Expected (±1min max))`
  );
});

test("Tehran method - sample accuracy validation", () => {
  const testCases = tehranTestData.testCases.slice(0, 10) as TestCase[];
  const results: {
    date: string;
    prayer: string;
    status: string;
  ...
  for (const testCase of testCases) {
    ...
    const config = {
      method: testCase.input.method as "Tehran",
  ...
  console.log(`Tehran Method Accuracy Analysis (${totalTests} tests):`);
  console.log(
...
test("Tehran method - edge cases and seasonal variation", () => {
  const coords = tehranTestData.metadata.coordinates;
  ...
  const config = {
    ...
    location,
    timezone: tehranTestData.metadata.timezone,
  };
  ...
  console.log(
    `✅ Tehran method passed edge case validation for ${keyDates.length} key dates`
  );
});
