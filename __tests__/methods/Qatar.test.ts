/**
 * Qatar method accuracy tests
 * Validates against official Qatar calculation authority data
 */

import { test, expect } from "bun:test";
import { PrayerTimeCalculator } from "../../src/api/prayer-times";
import qatarTestData from "../../test-data/Qatar.json";
...
test("Qatar method - metadata validation", () => {
  expect(qatarTestData.metadata.authority).toBe(
    "Qatar Architecture & Heritage"
  );
  expect(qatarTestData.metadata.method).toBe("Qatar");
  expect(qatarTestData.metadata.location).toBe("Doha, Qatar");
  expect(qatarTestData.testCases.length).toBeGreaterThan(0);

  console.log(
    `Qatar test data: ${qatarTestData.testCases.length} test cases (Official Authority Data - High Quality (±5min max))`
  );
});

test("Qatar method - sample accuracy validation", () => {
  const testCases = qatarTestData.testCases.slice(0, 10) as TestCase[];
  const results: {
    date: string;
    prayer: string;
    status: string;
  ...
  for (const testCase of testCases) {
    ...
    const config = {
      method: testCase.input.method as "Qatar",
  ...
  console.log(`Qatar Method Accuracy Analysis (${totalTests} tests):`);
  console.log(
...
test("Qatar method - full dataset statistical analysis", () => {
  const testCases = qatarTestData.testCases as TestCase[];
  let totalDifference = 0;
  let maxDifference = 0;
  let testCount = 0;

  const coords = qatarTestData.metadata.coordinates;
  const location: [number, number] = Array.isArray(coords)
    ? [coords[0], coords[1]]
    : [coords.latitude, coords.longitude];

  const config = {
    method: "Qatar" as const,
    location,
    timezone: qatarTestData.metadata.timezone,
  };
  ...
  console.log(`Qatar Statistical Analysis (${testCount} prayer times):`);
...
test("Qatar method - edge cases and seasonal variation", () => {
  const coords = qatarTestData.metadata.coordinates;
  const location: [number, number] = Array.isArray(coords)
    ? [coords[0], coords[1]]
    : [coords.latitude, coords.longitude];
  ...
  console.log(
    `✅ Qatar method passed edge case validation for ${keyDates.length} key dates`
  );
});
