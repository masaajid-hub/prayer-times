/**
 * Egypt method accuracy tests
 * Validates against Egyptian General Authority of Survey data
 */

import { test, expect } from "bun:test";
import { PrayerTimeCalculator } from "../../src/api/prayer-times";
import egyptTestData from "../../test-data/Egypt.json";
...
test("Egypt method - metadata validation", () => {
  expect(egyptTestData.metadata.authority).toBe(
    "Egyptian General Authority of Survey (ESA)"
  );
  expect(egyptTestData.metadata.method).toBe("Egypt");
  expect(egyptTestData.metadata.location).toBe("Cairo, Egypt");
  expect(egyptTestData.testCases.length).toBeGreaterThan(0);

  console.log(
    `Egypt test data: ${egyptTestData.testCases.length} test cases (Official Authority Data - Standard Quality (±5min max))`
  );
});

test("Egypt method - sample accuracy validation", () => {
  const testCases = egyptTestData.testCases.slice(0, 10) as TestCase[];
  const results: {
    date: string;
    prayer: string;
    status: string;
  ...
  for (const testCase of testCases) {
    ...
    const config = {
      method: testCase.input.method as "Egypt",
  ...
  console.log(`Egypt Method Accuracy Analysis (${totalTests} tests):`);
  console.log(
...
test("Egypt method - full dataset statistical analysis", () => {
  const testCases = egyptTestData.testCases as TestCase[];
  let totalDifference = 0;
  let maxDifference = 0;
  let testCount = 0;

  const coords = egyptTestData.metadata.coordinates;
  const location: [number, number] = Array.isArray(coords)
    ? [coords[0], coords[1]]
    : [coords.latitude, coords.longitude];

  const config = {
    method: "Egypt" as const,
    location,
    timezone: egyptTestData.metadata.timezone,
  };
  ...
  console.log(`Egypt Statistical Analysis (${testCount} prayer times):`);
...
test("Egypt method - edge cases and seasonal variation", () => {
  const coords = egyptTestData.metadata.coordinates;
  const location: [number, number] = Array.isArray(coords)
    ? [coords[0], coords[1]]
    : [coords.latitude, coords.longitude];
  ...
  console.log(
    `✅ Egypt method passed edge case validation for ${keyDates.length} key dates`
  );
});
