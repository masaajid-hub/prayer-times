...
import dubaiTestData from "../../test-data/Dubai.json";
...
test("Dubai method - metadata validation", () => {
  expect(dubaiTestData.metadata.authority).toBe(
    "General Authority of Islamic Affairs & Endowments, UAE"
  );
  expect(dubaiTestData.metadata.method).toBe("Dubai");
  expect(dubaiTestData.metadata.location).toBe("Dubai, UAE");
  expect(dubaiTestData.testCases.length).toBeGreaterThan(0);

  console.log(
    `Dubai test data: ${dubaiTestData.testCases.length} test cases (Official Authority Data - Standard Quality (±5min max))`
  );
});

test("Dubai method - sample accuracy validation", () => {
  const testCases = dubaiTestData.testCases.slice(0, 10) as TestCase[];
  const results: {
    date: string;
    prayer: string;
    status: string;
  ...
  for (const testCase of testCases) {
    ...
    const config = {
      method: testCase.input.method as "Dubai",
  ...
  console.log(`Dubai Method Accuracy Analysis (${totalTests} tests):`);
  console.log(
...
test("Dubai method - full dataset statistical analysis", () => {
  const testCases = dubaiTestData.testCases as TestCase[];
  let totalDifference = 0;
  let maxDifference = 0;
  let testCount = 0;

  const coords = dubaiTestData.metadata.coordinates;
  ...
  const config = {
    ...
    location,
    timezone: dubaiTestData.metadata.timezone,
  };
  ...
  console.log(`Dubai Statistical Analysis (${testCount} prayer times):`);
...
test("Dubai method - edge cases and seasonal variation", () => {
  const coords = dubaiTestData.metadata.coordinates;
  const location: [number, number] = Array.isArray(coords)
    ? [coords[0], coords[1]]
    : [coords.latitude, coords.longitude];
  ...
  console.log(
    `✅ Dubai method passed edge case validation for ${keyDates.length} key dates`
  );
});
