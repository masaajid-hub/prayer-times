/**
 * France15 method accuracy tests
...
import france15TestData from "../../test-data/France15.json";
...
test("France15 method - metadata validation", () => {
  expect(france15TestData.metadata.method).toBe("France15");
  expect(france15TestData.testCases.length).toBeGreaterThan(0);

  console.log(
    `France15 test data: ${france15TestData.testCases.length} test cases (Reference Implementation - Exact Match Expected (±1min max))`
  );
});

test("France15 method - sample accuracy validation", () => {
  const testCases = france15TestData.testCases.slice(0, 10) as TestCase[];
  const results: {
    date: string;
    prayer: string;
    status: string;
  ...
  for (const testCase of testCases) {
    ...
    const config = {
      method: testCase.input.method as "France15",
  ...
  console.log(`France15 Method Accuracy Analysis (${totalTests} tests):`);
  console.log(
...
test("France15 method - edge cases and seasonal variation", () => {
  const coords = france15TestData.metadata.coordinates;
  const location: [number, number] = Array.isArray(coords)
    ? [coords[0], coords[1]]
    : [coords.latitude, coords.longitude];

  const config = {
    method: "France15" as const,
    location,
    timezone: france15TestData.metadata.timezone,
  };
  ...
  console.log(
    `✅ France15 method passed edge case validation for ${keyDates.length} key dates`
  );
});
