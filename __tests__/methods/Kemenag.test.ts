...
import kemenagTestData from "../../test-data/Kemenag.json";
...
test("Kemenag method - metadata validation", () => {
  expect(kemenagTestData.metadata.authority).toBe(
    "Kementerian Agama Republik Indonesia"
  );
  expect(kemenagTestData.metadata.method).toBe("Kemenag");
  expect(kemenagTestData.metadata.location).toBe("Jakarta, Indonesia");
  expect(kemenagTestData.testCases.length).toBeGreaterThan(0);

  console.log(
    `Kemenag test data: ${kemenagTestData.testCases.length} test cases (Official Authority Data - Standard Quality (±5min max))`
  );
});

test("Kemenag method - sample accuracy validation", () => {
  const testCases = kemenagTestData.testCases.slice(0, 10) as TestCase[];
  const results: {
    date: string;
    prayer: string;
    status: string;
  ...
  for (const testCase of testCases) {
    ...
    const config = {
      method: testCase.input.method as "Kemenag",
  ...
  console.log(`Kemenag Method Accuracy Analysis (${totalTests} tests):`);
  console.log(
...
test("Kemenag method - full dataset statistical analysis", () => {
  const testCases = kemenagTestData.testCases as TestCase[];
  let totalDifference = 0;
  let maxDifference = 0;
  let testCount = 0;

  const coords = kemenagTestData.metadata.coordinates;
  ...
  const config = {
    ...
    location,
    timezone: kemenagTestData.metadata.timezone,
  };
  ...
  console.log(`Kemenag Statistical Analysis (${testCount} prayer times):`);
...
test("Kemenag method - edge cases and seasonal variation", () => {
  const coords = kemenagTestData.metadata.coordinates;
  const location: [number, number] = Array.isArray(coords)
    ? [coords[0], coords[1]]
    : [coords.latitude, coords.longitude];
  ...
  console.log(
    `✅ Kemenag method passed edge case validation for ${keyDates.length} key dates`
  );
});
