/**
 * UmmAlQura method accuracy tests
...
import ummalquraTestData from "../../test-data/UmmAlQura.json";
...
// Official Authority Data - Special Case (±5min max) configuration
const TEST_CONFIG = {
  MAX_TOLERANCE: 5,
  TIER1_ACCURACY_TARGET: 0.3,
  TIER2_ACCURACY_TARGET: 0.6,
...
test("UmmAlQura method - metadata validation", () => {
  expect(ummalquraTestData.metadata.authority).toBe(
    "Umm Al-Qura University, Saudi Arabia"
  );
  expect(ummalquraTestData.metadata.method).toBe("UmmAlQura");
  expect(ummalquraTestData.metadata.location).toBe("Riyadh, Saudi Arabia");
  expect(ummalquraTestData.testCases.length).toBeGreaterThan(0);

  console.log(
    `UmmAlQura test data: ${ummalquraTestData.testCases.length} test cases (Official Authority Data - Special Case (±5min max))`
  );
});

test("UmmAlQura method - sample accuracy validation", () => {
  const testCases = ummalquraTestData.testCases.slice(0, 10) as TestCase[];
  const results: {
    date: string;
    prayer: string;
    status: string;
  ...
  for (const testCase of testCases) {
    ...
    const config = {
      method: testCase.input.method as "UmmAlQura",
  ...
  console.log(`UmmAlQura Method Accuracy Analysis (${totalTests} tests):`);
  console.log(
...
test("UmmAlQura method - full dataset statistical analysis", () => {
  const testCases = ummalquraTestData.testCases as TestCase[];
  let totalDifference = 0;
  let maxDifference = 0;
  let testCount = 0;

  const coords = ummalquraTestData.metadata.coordinates;
  ...
  const config = {
    ...
    location,
    timezone: ummalquraTestData.metadata.timezone,
  };
  ...
  console.log(`UmmAlQura Statistical Analysis (${testCount} prayer times):`);
...
test("UmmAlQura method - edge cases and seasonal variation", () => {
  const coords = ummalquraTestData.metadata.coordinates;
  const location: [number, number] = Array.isArray(coords)
    ? [coords[0], coords[1]]
    : [coords.latitude, coords.longitude];
  ...
  console.log(
    `✅ UmmAlQura method passed edge case validation for ${keyDates.length} key dates`
  );
});
