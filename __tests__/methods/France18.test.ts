/**
 * France18 method accuracy tests
...
import france18TestData from "../../test-data/France18.json";
...
test("France18 method - metadata validation", () => {
  expect(france18TestData.metadata.method).toBe("France18");
  expect(france18TestData.testCases.length).toBeGreaterThan(0);

  console.log(
    `France18 test data: ${france18TestData.testCases.length} test cases (Reference Implementation - Exact Match Expected (±1min max))`
  );
});

test("France18 method - sample accuracy validation", () => {
  const testCases = france18TestData.testCases.slice(0, 10) as TestCase[];
  const results: {
    date: string;
    prayer: string;
    status: string;
  ...
  for (const testCase of testCases) {
    ...
    const config = {
      method: testCase.input.method as "France18",
  ...
  console.log(`France18 Method Accuracy Analysis (${totalTests} tests):`);
  console.log(
...
test("France18 method - edge cases and seasonal variation", () => {
  const coords = france18TestData.metadata.coordinates;
  const location: [number, number] = Array.isArray(coords)
    ? [coords[0], coords[1]]
    : [coords.latitude, coords.longitude];

  const config = {
    method: "France18" as const,
    location,
    timezone: france18TestData.metadata.timezone,
  ...
  console.log(
    `✅ France18 method passed edge case validation for ${keyDates.length} key dates`
  );
});
