...
import russiaTestData from "../../test-data/Russia.json";
...
test("Russia method - metadata validation", () => {
  expect(russiaTestData.metadata.authority).toBe(
    "Spiritual Administration of Muslims in Russia"
  );
  expect(russiaTestData.metadata.method).toBe("Russia");
  expect(russiaTestData.metadata.location).toBe("Moscow, Russia");
  expect(russiaTestData.testCases.length).toBeGreaterThan(0);

  console.log(
    `Russia test data: ${russiaTestData.testCases.length} test cases (Official Authority Data - Standard Quality (±5min max))`
  );
});

test("Russia method - sample accuracy validation", () => {
  const testCases = russiaTestData.testCases.slice(0, 10) as TestCase[];
  const results: {
    date: string;
    prayer: string;
    status: string;
  ...
  for (const testCase of testCases) {
    ...
    const config = {
      method: testCase.input.method as "Russia",
  ...
  console.log(`Russia Method Accuracy Analysis (${totalTests} tests):`);
  console.log(
...
test("Russia method - full dataset statistical analysis", () => {
  const testCases = russiaTestData.testCases as TestCase[];
  let totalDifference = 0;
  let maxDifference = 0;
  let testCount = 0;

  const coords = russiaTestData.metadata.coordinates;
  const location: [number, number] = Array.isArray(coords)
    ? [coords[0], coords[1]]
    : [coords.latitude, coords.longitude];

  const config = {
    method: "Russia" as const,
    location,
    timezone: russiaTestData.metadata.timezone,
  };
  ...
  console.log(`Russia Statistical Analysis (${testCount} prayer times):`);
...
test("Russia method - edge cases and seasonal variation", () => {
  const coords = russiaTestData.metadata.coordinates;
  const location: [number, number] = Array.isArray(coords)
    ? [coords[0], coords[1]]
    : [coords.latitude, coords.longitude];
  ...
  console.log(
    `✅ Russia method passed edge case validation for ${keyDates.length} key dates`
  );
});
